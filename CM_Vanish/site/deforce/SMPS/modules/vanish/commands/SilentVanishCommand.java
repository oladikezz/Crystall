...
public class SilentVanishCommand implements CommandExecutor, TabCompleter {
   private final SM_Vanish module;

   public SilentVanishCommand(SM_Vanish module) {
      super();
      this.module = module;
   }

   public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
      this.module.log("Silent vanish command from " + sender.getName());
      Player target;
      if (args.length == 0) {
         ...
         if (target == null) {
            ...
            if (newState) {
               sender.sendMessage(this.module.formatMessage("&7Silent Vanish for &e" + playerName + " &7(offline) &aENABLED"));
               sender.sendMessage(this.module.formatMessage("&7Игрок зайдёт в ванише."));
            } else {
               sender.sendMessage(this.module.formatMessage("&7Silent Vanish for &e" + playerName + " &7(offline) &cDISABLED"));
            }
      ...
      this.module.toggleSilentVanish(target);
      boolean isVanished = this.module.isVanished(target);
      if (!target.equals(sender)) {
         if (isVanished) {
            sender.sendMessage(this.module.formatMessage("&7Silent Vanish for &e" + target.getName() + " &aENABLED"));
         } else {
            sender.sendMessage(this.module.formatMessage("&7Silent Vanish for &e" + target.getName() + " &cDISABLED"));
         }
      }
